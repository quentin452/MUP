...
public abstract class MixinNetHandlerPlayServer
{
    ...
    {
        if (Mup.config.dac.enabled && shouldCheckPlayerMovement())
        {
            handlePlayerMovedTooQuickly(d7, d8, d9);
        }
    }
    ...
    {
        return !shouldCheckPlayerMovement() || player.isInvulnerableDimensionChange();
    }
    ...
    @ModifyVariable(method = "processVehicleMove", index = 26, name = "d10", at = @At(value = "STORE", ordinal = 0),
            slice = @Slice(to = @At(value = "CONSTANT", args = "stringValue={} (vehicle of {}) moved too quickly! {},{},{}")))
    ...
    private boolean shouldCheckPlayerMovement()
    {
        return !this.player.isInvulnerableDimensionChange() && (!this.player.getServerWorld().getGameRules().getBoolean("disableElytraMovementCheck") || !this.player.isElytraFlying());
    }

    private void handlePlayerMovedTooQuickly(double d7, double d8, double d9)
    {
        float f2 = this.player.isElytraFlying() ? 300.0F : 100.0F;

        if (d9 - d8 > (double) (f2 * 0.6F) && (!this.server.isSinglePlayer() || !this.server.getServerOwner().equals(this.player.getName())))
        {
